...
def plot_to_matrix(tr2, tr4, tr6, tr8, tr1, trmse, trmae):
    ...
    with open("./allval.csv", 'a') as myfile:
...
def averageResults(stringTofind):
    ...
    for file in files:
        [testdbcur, tr2cur, tr4cur, tr6cur, tr8cur, tr1cur, trMseCur, trMaeCur] = CropFile(file)
    ...
    return [finaldb, finaltr2, finaltr4, finaltr6, finaltr8, finaltr1, finalmsetr, finalmaetr]
...
def CropFile(file):
    ...
    trMsePattern = re.compile(".*Validation MSE:.*")
    ...
    for i, line in enumerate(inputfile):
        ...
        line = line.replace("\n", "")
        if (trMseCounter == 2):
            trmse.append(line.replace("  +Validation MSE: ",""))
            trMseCounter -= 1
        elif (trMseCounter == 1):
            trmae.append(line.replace("  +Validation MAE: ",""))
            trMseCounter -= 1
    ...
    return [testdb, trdb2, trdb4, trdb6, trdb8, trdb1, trmse, trmae ]
...
def ParseDataFileSingleChannel(file):
    ...
    for line in my_text:
        ...
        if line.find("after adding") != -1:
            ...
            for i in range(0, len(list), 2): # 56:-18.2, 80:-17.9, 60:-18.0, 4:-20.7, |28|:-18.4
                set_number += 1
                listChan = int(list[i])
            ...
            for i in range(0, len(list), 2):
                listChan = int(list[i])
                inputsDict[listChan].append(np.array(out_power))
                outputEntry[0] = pow(10, float(list[i + 1]) / 10)
                outputsDict[listChan].append(np.array(outputEntry))
                counter_all.append(set_number)
            currentChannel = -1
